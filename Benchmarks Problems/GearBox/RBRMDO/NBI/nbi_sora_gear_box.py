...
def g(d, x, p):
    ...
    g5 = 1100. - np.sqrt((745*x[3]/(x[1]*x[2]))**2 + 16.9e6) / (0.10 * x[5]**3)
...
betaT = np.array([3., 3., 3., 3., 3., 3., 3., 3., 3., 3., 3.])
...
def Sf1(d, x):
    Vmin = f1(d, x)
    return Vmin / Vmin0
d1, x1, r1 = SORA(Sf1, g, betaT, x = x, bndsX = bndsX)

# SUBSTITUTE OBJECTIVE FUNCTION 2
def Sf2(d, x):
    Smax = f2(d, x)
    return Smax / Smax0
d2, x2, r2 = SORA(Sf2, g, betaT, x = x, bndsX = bndsX)
...
for w1, w2 in zip(W1, W2):
    if w1 + w2 == 1.:
        if w1 == 1.:
            ...
            d, x, res = SORA(f, g, betaT, x = x_i, bndsX = bndsX_0, cons = const_nbi, bnds_cons = bnds_cons)
