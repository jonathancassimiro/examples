...
def lsf(x, g):
    ...
    stochastic_model.addVariable(Lognormal('x1', x[0], 30.))
    stochastic_model.addVariable(Lognormal('x2', x[1], 2.))
    stochastic_model.addVariable(Lognormal('x3', x[2], 50.))
    stochastic_model.addVariable(Lognormal('p1', 400., 35.))
    stochastic_model.addVariable(Normal('p2', 30., 10.))
    stochastic_model.addVariable(Weibull('p3', 2.1e4, 4.2e3))
    stochastic_model.addVariable(Normal('p4', 5e5, 5e4))
    stochastic_model.addVariable(Gumbel('p5', 6e5, 9e4))
    stochastic_model.addVariable(Gumbel('p6', 6e5, 2e5))
...
def g1(x1, x2, x3, p1, p2, p3, p4, p5, p6):
    ...
    As = 2. * x1 * x2
    ms = x1 * x2 * x3
    eb = np.pi**2 * p3 * x1 * x2 * x3**2 / (2. * (7500)**2)
    return p1 - F * ((1. / As) + (p2 / ms) * (eb / (eb - F)))
