...
def SORA(f, df, g, betaT, d = [], x = [], p = [], R = None, bndsd = None, bndsX = None, bnds_cons = None, cons = None, maxciclo = 50, ftol = 1e-4, gtol = 1e-4, MPPmaxiter = 50, MPPtol = 1e-4, disp = False, OPTmaxiter = 1000, OPTtol = 1e-4):
    ...
    def optimization(VP):
        res = minimize(fun=faux, jac = dfaux, x0=VP, method='slsqp', bounds=bnds, constraints=constr, options={'disp': True, 'maxiter': OPTmaxiter, 'ftol': OPTtol})
        d = res.x[:nVDt]
        mean_x = res.x[nVDt:]
        VP = np.concatenate((d, mean_x))
        f_new = res.fun
    ...
    # Convergence history
    def history(k, d, mean_x, f_new, xMPP, pMPP, s, VP):
        ...
        print('pMPP: ', pMPP)
        print('s: ', s)
        print('g(d, mean_x - s, pMPP): ', cons_aux(VP))
        if ncons != 0:
    ...
    while k <= maxciclo:
        ...
        for i in range(nLS):
            if k == 1:
                xMPP[i, :], pMPP[i, :] = MPP(i, betaT[i], d, mean_x, mean_p, x)
            else:
                xMPP[i, :], pMPP[i, :] = MPP(i, betaT[i], d, xMPP[i, :], pMPP[i, :], x)
            LSval[i] = g(d, xMPP[i, :], pMPP[i, :])[i]

        # Convergence history
        if disp == True:
            history(k, d, mean_x, f_new, xMPP, pMPP, s, VP)
