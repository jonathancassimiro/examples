...
class Form(AnalysisObject):
    ...
    def run(self):
        ...
        while not convergence:
            ...
            # Check convergence
            if self.G == 0 or self.Go == 0:
                e1 = 0
            else:
                e1 = np.absolute(self.G * self.Go ** (-1))[0]
            lu = np.linalg.norm(self.u)
            if lu == 0:
                lu = 1
            e2 = np.linalg.norm(self.u / lu - self.alpha)
            if self.options.is_PMA:
                condition1 = True
            else:
            ...
            if not convergence:
                ...
                if self.options.is_PMA:
                    u_new = self.alpha * self.options.beta_targ
                else:
    ...
    def computeJacobian(self):
        """Compute the Jacobian"""
        J_u_x = self.transform.jacobian(self.u, self.x, self.model.getMarginalDistributions())

        # Verificando se a matriz J_u_x é singular
        if np.isclose(np.linalg.det(J_u_x), 0):
            # Regularizando a matriz J_u_x
            epsilon = 1e-8
            J_u_x = J_u_x + np.eye(J_u_x.shape[0]) * epsilon
