...
class VA:
    ...
    def __init__(self, dist, **kwargs):
        if dist not in ['Uniform', 'Normal', 'Lognormal', 'Exponential', 'Rayleigh', 'Logistic', 'Gamma', 'Gumbel_MIN', 'Gumbel_MAX', 'Weibull_MIN', 'Weibull_MAX']:
    ...
    def calcpar(self):
        ...
        if (self.dist == 'Uniform'):
            self.par1 = self.mean - np.sqrt(3.) * self.sdev
            self.par2 = self.mean + np.sqrt(3.) * self.sdev
            ...
            if self.mean == 0.:
                ...
                self.par2 = np.sqrt(np.log(1. + (self.sdev / self.mean) ** 2))
                self.par1 = np.log(self.mean) - 0.5 * self.par2 ** 2
        elif (self.dist == 'Exponential'):
            self.par1 = 1./self.sdev
            self.par3 = self.mean - self.sdev
        elif (self.dist == 'Rayleigh'):
            self.par1 = self.sdev/np.sqrt(2. - pi/2.)
            self.par3 = self.mean - self.par1 * np.sqrt(pi / 2.)
        elif (self.dist == 'Logistic'):
            self.par1 = self.mean
            self.par2 = self.sdev * np.sqrt(3.) / pi
        elif (self.dist == 'Gamma'):
            self.par1 = (self.mean / self.sdev) ** 2
            self.par2 = self.mean / self.sdev ** 2
        elif (self.dist == 'Gumbel_MIN'):
            self.par2 = np.pi / (np.sqrt(6.) * self.sdev)
            self.par1 = self.mean + 0.577216 / self.par2
        elif (self.dist == 'Gumbel_MAX'):
            self.par2 = np.pi / (np.sqrt(6.) * self.sdev)
            self.par1 = self.mean - 0.577216 / self.par2
        elif (self.dist == 'Weibull_MIN'):
            self.par2 = root(lambda x: 1. + (self.sdev / (self.mean - self.vmin))**2 - gamma(1. + 2. / x) / (gamma(1. + 1./ x))**2, x0 = self.mean, method='hybr').x[0]
            self.par1 = (self.mean - self.vmin) / gamma(1. + 1. / self.par2) + self.vmin
        elif (self.dist == 'Weibull_MAX'):
            self.par2 = root(lambda x: 1. + (self.sdev / (self.vmax - self.mean))**2 - gamma(1. + 2. / x) / (gamma(1. + 1./ x))**2, x0 = self.mean, method='hybr').x[0]
            self.par1 = (self.mean - self.vmax) / gamma(1. + 1. / self.par2) + self.vmax
    ...
    def pdf(self, x):
        if (self.dist == 'Uniform'):
            ...
            if self.mean == 0.:
                ...
                return phi((np.log(x) - self.par1) / self.par2) / (x * self.par2)
        elif (self.dist == 'Exponential'):
            if (x < self.par3):
                return 0.
            else:
                return self.par1 * np.exp(-self.par1 * (x - self.par3))
        elif (self.dist == 'Rayleigh'):
            if (x < self.par3):
                return 0.
            else:
                return (x - self.par3) / self.par1 ** 2 * np.exp(-1./2. * ((x - self.par3) / self.par1) ** 2)
        elif (self.dist == 'Logistic'):
            return np.exp(-(x - self.par1) / self.par2) / (self.par2 * (1. + np.exp(-(x - self.par1) / self.par2)) ** 2)
        elif (self.dist == 'Gamma'):
            if (x < 0.):
                return 0.
            else:
                return self.par2 * (self.par2 * x) ** (self.par1 - 1) * np.exp(-self.par2 * x) / gamma(self.par1)
        elif (self.dist == 'Gumbel_MIN'):
            return self.par2 * np.exp(self.par2 * (x - self.par1) - np.exp( self.par2 * (x - self.par1)))
        elif (self.dist == 'Gumbel_MAX'):
            return self.par2 * np.exp(self.par2 * (x - self.par1) - np.exp(-self.par2 * (x - self.par1)))
        elif (self.dist == 'Weibull_MIN'):
            if (x < self.vmin):
                ...
                return (self.par2 / (self.par1 - self.vmin)) * ((x - self.vmin) / (self.par1 - self.vmin))**(self.par2 - 1) * np.exp(-((x - self.vmin) / (self.par1 - self.vmin))**self.par2)
        elif (self.dist == 'Weibull_MAX'):
                if (x > self.vmin):
                    return 0.
                else:
                    return (self.par2 / (self.vmax - self.par1)) * ((self.vmax - x) / (self.vmax - self.par1)) ** (self.par2 - 1) * np.exp(-(-((x - self.vmax) / (self.par1 - self.vmax))**self.par2))
    ...
    def cdf(self, x):
        if (self.dist == 'Uniform'):
            ...
            if self.mean == 0. or x < 0.:
                ...
                return Phi((np.log(x) - self.par1) / self.par2)
        elif (self.dist == 'Exponential'):
            if (x < self.par3):
                return 0.
            else:
            ...
            if (x < self.par3):
                ...
                return 1. - np.exp(-1./2. * ((x - self.par3) / self.par1) ** 2)
        elif (self.dist == 'Logistic'):
            return 1./(1. + np.exp(-(x - self.par1) / self.par2))
        elif (self.dist == 'Gamma'):
            if (x < 0.):
                return 0.
            else:
            ...
            return 1. - np.exp(-np.exp(self.par2 * (x - self.par1)))
        elif (self.dist == 'Gumbel_MAX'):
            return np.exp(-np.exp(-self.par2 * (x - self.par1)))
        elif (self.dist == 'Weibull_MIN'):
            if (x < self.vmin):
                return 0.
            else:
                return 1. - np.exp(-((x - self.vmin) / (self.par1 - self.vmin))**self.par2)
        elif (self.dist == 'Weibull_MAX'):
                if (x > self.vmax):
                    return 1.
                else:
                    return np.exp(-((self.vmax - x) / (self.vmax - self.par1))**self.par2)
    ...
    def invcdf(self, Fx):
        if (self.dist == 'Uniform'):
            ...
            return np.exp(self.par2 * InvPhi(Fx) + self.par1)
        elif (self.dist == 'Exponential'):
            return (self.par3 - np.log(1. - Fx)) / self.par1
        elif (self.dist == 'Rayleigh'):
            return self.par3 + self.par1 * np.sqrt(-2. * np.log(1. - Fx))
        elif (self.dist == 'Logistic'):
            return self.par1 - self.par2 * np.log(1./Fx - 1.)
        elif (self.dist == 'Gamma'):
            return optimize.root(lambda x: gammainc(self.par1, x * self.par2) - Fx, x0 = self.mean, method='hybr').x[0]
        elif (self.dist == 'Gumbel_MIN'):
            return self.par1 + np.log(-np.log(1. - Fx)) / self.par2
        elif (self.dist == 'Gumbel_MAX'):
            return self.par1 - np.log(-np.log(Fx)) / self.par2
        elif (self.dist == 'Weibull_MIN'):
            return self.vmin + (self.par1 - self.vmin) * (-np.log(1 - Fx)**(1. / self.par2))
        elif (self.dist == 'Weibull_MAX'):
            return self.vmax - (self.vmax - self.par1) * (-np.log(Fx)**(1. / self.par2))
...
def SLA(f, g, betaT, d = [], x = [], p = [], R = None, bndsd = None, bndsX = None, bnds_cons = None, cons = None, maxciclo = 50, ftol = 1e-4, disp = False, OPTmaxiter = 1000, OPTtol = 1e-4):
    ...
    def optimization(VP):
        res = minimize(fun=faux, x0=VP, method='slsqp', bounds=bnds, constraints=constr,
