...
for i in range(24):
    ...
    # OBJECTIVE FUNCTION - TOTAL COST CONSTRUCTION
    def fcost(x):
    ...
    # OBJECTIVE FUNCTION - MAXIMUM ROAD LENGTH
    def Lmax(x):

        h, As_center, As_edge, As_shrinkage = x[0]/1e2, x[1]/1e4, x[2]/1e4, x[3]/1e4

        # rigid pavement volume of concrete per meter
        Vrp = h * A + d*(so+si)

        # CA-60 weight of steel per meter
        Wsw = (1/B) * ((B * As_center * (A - 2*c) + A * As_center * (B - 2*c)) + (B * As_shrinkage * (A - 2*c) + A * As_shrinkage * (B - 2*c)) + (1.6 * As_edge * (B - 2*c))) * ρs
        ...
        Ws = C * (25 * ((1/B) - 1) * (np.pi * (phi_tb)**2 / 4) + 16 * (1/B) * (np.pi * (phi_lb)**2 / 4)) * ρs
    ...
    # OPTIMIZATIONS
    res1 = minimize(fun=fcost, x0=x0, method='slsqp', bounds=bnds, constraints=cons, options={'disp': False, 'maxiter':1000, 'ftol': 1e-4})
    fcost_opt_list.append(res1.fun)
    xcost_opt_list.append(res1.x)

    res2 = minimize(fun=Lmax, x0=x0, method='slsqp', bounds=bnds, constraints=cons, options={'disp': False, 'maxiter':1000, 'ftol': 1e-4})
    Lmax_opt_list.append(res1.fun)
    xmax_opt_list.append(res1.x)

# RESULTS
for i in range(24):
    print(f"f = {fcost_opt_list[i]:.2f}, x = {xcost_opt_list[i]}")
    print(fcost_opt_list)
    print(xcost_opt_list)

    print(f"f = {Lmax_opt_list[i]:.2f}, x = {xmax_opt_list[i]}")
    print(Lmax_opt_list)
    print(xmax_opt_list)